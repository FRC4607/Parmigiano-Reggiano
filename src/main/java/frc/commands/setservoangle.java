// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.commands;

import edu.wpi.first.wpilibj2.command.Command;
import edu.wpi.first.wpilibj2.command.button.CommandXboxController;
import frc.robot.Robot;
import frc.robot.subsystems.Launcher;

/* You should consider using the more terse Command factories API instead https://docs.wpilib.org/en/stable/docs/software/commandbased/organizing-command-based.html#defining-commands */
public class setservoangle extends Command {
  Launcher m_Launcher;
  CommandXboxController m_controller;
  long buttonlastpesseed;
  /** Creates a new setservoangle. */
  public setservoangle(CommandXboxController controller, Launcher launcher) {
    m_Launcher = launcher;
    m_controller = controller;
    buttonlastpesseed = 0;
    addRequirements(m_Launcher);

    // Use addRequirements() here to declare subsystem dependencies.
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {}

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    if (m_controller.a().getAsBoolean()){
     buttonlastpesseed = Robot.timercount;
    }
    if (buttonlastpesseed != 0 && buttonlastpesseed > Robot.timercount - 50){
      m_Launcher.setServoAngle(0.0);
    }else {
      m_Launcher.setServoAngle(65.0);

    }
  }

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {}

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    return false;
  }
}
